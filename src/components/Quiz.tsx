...
const QUESTIONS: Question[] = [
  {
    ...
    options: [
      { label: "Minha cabeça vive cansada", category: "overload", weight: 3, signal: "Sua cabeça parece cansar antes mesmo do dia terminar" },
      { label: "Acordo sem energia", category: "rest", weight: 3, signal: "Você acorda como se não tivesse recuperado de verdade" },
      { label: "Me sinto sobrecarregado(a)", category: "overload", weight: 2, signal: "As tarefas simples parecem pesar mais do que deveriam" },
      { label: "Me sinto normal", category: "focus", weight: 0, signal: "Seu dia parece mais estável na maior parte do tempo" },
    ],
  },
  {
    id: "memory",
    ...
    options: [
      { label: "Esqueço direto", category: "focus", weight: 3, signal: "Você se pega esquecendo recados, nomes ou pequenas tarefas" },
      { label: "Às vezes acontece", category: "focus", weight: 2, signal: "Alguns esquecimentos aparecem quando a rotina aperta" },
      { label: "Quase nunca", category: "rest", weight: 0, signal: "Sua memória parece funcionar bem na maior parte dos dias" },
    ],
  },
  {
    id: "attention",
    ...
    options: [
      { label: "Me distraio fácil", category: "focus", weight: 3, signal: "Você começa uma coisa e logo percebe que foi puxado(a) para outra" },
      { label: "Tenho dificuldade pra manter o foco", category: "focus", weight: 2, signal: "Manter atenção por muito tempo tem exigido esforço extra" },
      { label: "Meu foco é normal", category: "rest", weight: 0, signal: "Seu foco parece acompanhar bem sua rotina" },
    ],
  },
  {
    id: "sleep",
    ...
    options: [
      { label: "Durmo e acordo cansado", category: "rest", weight: 3, signal: "Mesmo dormindo, você acorda com sensação de bateria baixa" },
      { label: "Demoro pra dormir", category: "rest", weight: 2, signal: "Sua mente demora para desligar quando chega a hora de descansar" },
      { label: "Durmo bem", category: "overload", weight: 0, signal: "Seu sono parece estar ajudando na recuperação" },
    ],
  },
  {
    id: "sleep-hours",
    ...
    options: [
      { label: "Durmo entre 7 e 8 horas", category: "rest", weight: 0, signal: "Você costuma dar ao corpo uma boa janela de descanso" },
      { label: "Durmo cerca de 6 horas", category: "focus", weight: 1, signal: "Seu descanso pode estar ficando um pouco abaixo do ideal" },
      { label: "Durmo menos de 6 horas", category: "rest", weight: 3, signal: "Poucas horas de sono podem pesar na energia e na clareza mental" },
      { label: "Meu horário de sono varia muito", category: "overload", weight: 2, signal: "Sua rotina de sono pode estar oscilando mais do que o corpo gostaria" },
    ],
  },
  {
    id: "nutrition",
    ...
    options: [
      { label: "Me alimento bem na maior parte dos dias", category: "focus", weight: 0, signal: "Sua alimentação parece apoiar bem sua rotina mental" },
      { label: "Tento manter minha alimentação boa", category: "focus", weight: 1, signal: "Você tenta cuidar da alimentação, mesmo com alguns altos e baixos" },
      { label: "Minha alimentação oscila bastante", category: "overload", weight: 2, signal: "Sua alimentação muda bastante conforme a rotina aperta" },
      { label: "Pulo refeições ou como no automático", category: "rest", weight: 3, signal: "Sua alimentação pode estar sofrendo quando o dia fica corrido" },
    ],
  },
  {
    id: "alcohol",
    ...
    options: [
      { label: "Quase não bebo", category: "rest", weight: 0, signal: "O álcool parece ter pouca influência na sua rotina de recuperação" },
      { label: "Bebo socialmente e tento manter controle", category: "overload", weight: 1, signal: "Você tenta manter o álcool dentro de uma rotina controlada" },
      { label: "Bebo para relaxar em alguns dias", category: "overload", weight: 2, signal: "Às vezes você usa o álcool como válvula de escape para aliviar a rotina" },
      { label: "Percebo que o álcool atrapalha meu sono", category: "rest", weight: 3, signal: "O álcool pode estar interferindo na qualidade do seu descanso" },
    ],
  },
  {
    id: "pressure",
    ...
    options: [
      { label: "Fico mentalmente travado(a)", category: "overload", weight: 3, signal: "Sob pressão, parece que sua mente trava ou fica lenta" },
      { label: "Perco o ritmo e procrastino", category: "focus", weight: 2, signal: "Você tenta render, mas se perde entre distrações e atrasos" },
      { label: "Sinto o corpo pesado", category: "rest", weight: 2, signal: "O corpo pesa junto com a mente nos dias mais puxados" },
      { label: "Consigo lidar bem", category: "overload", weight: 0, signal: "Você ainda sente controle mesmo em dias movimentados" },
    ],
  },
  {
    id: "activity",
    ...
    options: [
      { label: "Me movimento com frequência", category: "focus", weight: 0, signal: "O movimento já parece fazer parte do seu cuidado com energia e disposição" },
      { label: "Tento me exercitar durante a semana", category: "focus", weight: 1, signal: "Você tenta manter atividade física mesmo quando a rotina aperta" },
      { label: "Faço atividade só quando dá", category: "overload", weight: 2, signal: "Sua frequência de movimento pode cair quando o dia fica mais cheio" },
      { label: "Estou sedentário(a) no momento", category: "rest", weight: 3, signal: "A falta de movimento pode estar contribuindo para cansaço e baixa disposição" },
    ],
  },
];
...
> = {
  balanced: {
    ...
    reassurance: "Mesmo assim, manter esse equilíbrio exige cuidado contínuo, sono de qualidade e suporte nutricional adequado.",
  ...
  overload: {
    ...
    reassurance: "A boa notícia: pequenos ajustes e o suporte certo podem ajudar seu corpo a recuperar um ritmo mais leve.",
    recommendation: "pa",
  },
  focus: {
    ...
    reassurance: "Isso não significa falta de vontade. Muitas vezes o corpo só precisa de mais suporte para sustentar energia mental.",
    recommendation: "treonato",
  },
  rest: {
    ...
    reassurance: "Com uma rotina mais favorável e nutrientes adequados, é possível melhorar a qualidade desse descanso.",
    recommendation: "pa",
  },
};
...
export function Quiz() {
  ...
  function handleAnswer(option: Option) {
    ...
    setStage("loading");
    setTimeout(() => setStage("result"), 5600);
  }
  ...
  return (
    <main className="relative min-h-screen overflow-hidden">
      <div className="pointer-events-none absolute inset-0 -z-10">
        <div className="absolute -top-40 left-1/2 h-[600px] w-[600px] -translate-x-1/2 rounded-full bg-brand/30 blur-[120px]" />
        <div className="absolute bottom-0 right-0 h-[500px] w-[500px] rounded-full bg-brand-red/20 blur-[140px]" />
      </div>
      ...
      <section className="relative z-10 mx-auto flex min-h-[calc(100vh-100px)] max-w-3xl items-center justify-center px-6 pb-16">
        <AnimatePresence mode="wait">
          {stage === "intro" && <Intro key="intro" onStart={() => setStage("questions")} />}
          ...
          {stage === "result" && (
            <Result key="result" answers={answers} category={result} intensity={intensity} onRestart={reset} />
          )}
        </AnimatePresence>
      </section>
    </main>
    ...
    >
      <div className="mb-10">
        <div className="mb-3 flex items-center justify-between text-xs uppercase tracking-[0.25em] text-muted-foreground">
          <button onClick={onBack} className="flex items-center gap-2 transition hover:text-foreground">
        ...
        <div className="h-1.5 overflow-hidden rounded-full bg-white/5">
          <motion.div
            ...
            className="h-full rounded-full bg-gradient-to-r from-brand to-brand-red shadow-[0_0_20px] shadow-brand-red-glow"
          />
        </div>
      </div>
      ...
      <div className="mb-8 flex items-center gap-3">
        ...
        <span className="text-xs uppercase tracking-[0.3em] text-muted-foreground">{question.subtitle}</span>
      </div>
      ...
      <div className="grid gap-3">
        {question.options.map((option, index) => (
          <motion.button
            ...
            className="glass group flex items-center justify-between rounded-2xl px-6 py-5 text-left transition-all hover:border-brand-glow/50 hover:bg-brand/10"
          >
...
function Result({
  ...
  const identitySignals = answers.some((answer) => answer.weight > 0)
    ? answers.filter((answer) => answer.weight > 0).slice(0, 4)
    : answers.filter((answer) => answer.weight === 0).slice(0, 4);
  ...
  const [remainingOfferTime, setRemainingOfferTime] = useState(OFFER_DURATION_MS);
  const formattedPreviousPrice = formatPrice(recommendation.previousPrice);
  const formattedPrice = formatPrice(recommendation.price);
  const countdown = formatOfferTime(remainingOfferTime);
  ...
  useEffect(() => {
    const updateRemainingTime = () => {
      setRemainingOfferTime(Math.max(0, offerEndsAt - Date.now()));
    };
    ...
    <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="w-full">
      <div className="mb-6 text-center">
        ...
        <h2 className="font-display mt-5 text-4xl leading-tight sm:text-6xl">
          Seu resultado: <span className="bg-gradient-to-r from-brand to-brand-glow bg-clip-text text-transparent">{result.title}</span>
        </h2>
      </div>
      ...
      <div className="glass mb-6 rounded-3xl p-6">
        ...
        <p className="text-xs uppercase tracking-[0.3em] text-brand-glow">Com base nas suas respostas</p>
        <h3 className="font-display mt-3 text-3xl leading-tight sm:text-4xl">
          {result.identityTitle ?? "Identificamos alguns sinais comuns de sobrecarga mental e física no seu dia a dia."}
        </h3>
        ...
        <ul className="mt-5 space-y-3">
          {identitySignals.map((answer) => (
            <li key={`${answer.label}-${answer.signal}`} className="flex items-start gap-3 text-sm sm:text-base">
        ...
        <p className="mt-5 rounded-2xl border border-brand/30 bg-brand/10 p-4 text-sm font-medium text-foreground">
          {result.identityClosing ?? "Se você marcou mentalmente 2 ou mais pontos... seu corpo pode estar pedindo suporte."}
        </p>
      </div>

      <div className="glass relative overflow-hidden rounded-3xl p-8 sm:p-10">
        <div className="absolute -right-20 -top-20 h-72 w-72 rounded-full bg-brand-red/25 blur-[100px]" />
        <div className="relative grid gap-8 sm:grid-cols-[1fr_auto] sm:items-center">
          <div>
            <p className="text-xs uppercase tracking-[0.3em] text-brand-glow">O que pode ajudar no seu caso</p>
            <h3 className="font-display mt-2 text-4xl leading-none sm:text-5xl">{recommendation.name}</h3>
            <p className="mt-3 text-sm leading-relaxed text-muted-foreground">Com base no seu perfil, essa é uma das opções mais indicadas.</p>
          ...
          >
            <div className="absolute inset-0 rounded-full bg-brand-glow/40 blur-3xl" />
            <img
      ...
      <div className="mt-6 rounded-3xl border border-brand-red/40 bg-brand-red/10 p-6 text-center shadow-[0_24px_80px_-40px] shadow-brand-red-glow">
        <AnimatePresence mode="wait">
          {benefitState === "revealed" ? (
            ...
            >
              <p className="text-xs uppercase tracking-[0.3em] text-brand-red-glow">Condição exclusiva liberada</p>
              <h3 className="font-display mt-2 text-4xl text-brand-red-glow sm:text-5xl">30% de desconto</h3>
              <div className="mx-auto mt-4 max-w-md rounded-2xl border border-white/15 bg-black/20 p-5">
                <p className="text-xs uppercase tracking-[0.25em] text-white/55">{recommendation.name}</p>
                <div className="mt-3 flex flex-wrap items-end justify-center gap-x-4 gap-y-2">
                  ...
                  <span className="font-display text-5xl leading-none text-white sm:text-6xl">{formattedPrice}</span>
                </div>
              ...
              <p className="mx-auto mt-2 max-w-lg text-sm text-white/80">
                Válido para quem falar com o especialista agora e confirmar a recomendação personalizada Amevit.
              </p>
            </motion.div>
          ) : (
            <motion.div
            ...
            >
              <p className="text-xs uppercase tracking-[0.3em] text-brand-red-glow">Você deu um passo importante para sua saúde</p>
              <h3 className="font-display mt-2 text-3xl text-white sm:text-4xl">Tem uma condição especial esperando por você.</h3>
              <p className="mx-auto mt-2 max-w-lg text-sm text-white/80">
                Como você concluiu sua análise, a Amevit pode liberar um benefício exclusivo para começar seu cuidado com mais facilidade.
              </p>
              <motion.button
                whileHover={benefitState === "hidden" ? { scale: 1.03 } : undefined}
                whileTap={benefitState === "hidden" ? { scale: 0.97 } : undefined}
